 ...
 */
void dist_serial(vect_dist *const args) {
  double accum = 0.0;
  for (int i = args->start; i < args->end && i < args->nDim; i++) {
    accum += pow((args->x[i] - args->y[i]), 2);
  }
  args->accum = accum;
}

// Parallelized distance function by tracking accumulation per batch and then summing
double dist(double *x, double *y, int nDim) {
  int numThreads = 16;
  int batch_size = nDim / numThreads;
  vect_dist args[numThreads];
  for (int i=0; i<numThreads ; i++){
    args[i].x = x;
    args[i].y = y;
    args[i].nDim = nDim;
    args[i].start = i * batch_size;
    args[i].end = args[i].start + batch_size;
  }
  std::thread workers[numThreads];
  for (int i=1; i<numThreads; i++) {
      workers[i] = std::thread(dist_serial, &args[i]);
  }
  dist_serial(&args[0]);
  // join worker threads
  for (int i=1; i<numThreads; i++) {
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  int iter = 0;
  double startTime, stopTime, comp_assignments, comp_centroids, comp_cost;
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    stopTime = CycleTimer::currentSeconds();
    comp_assignments += (stopTime - startTime);
    startTime = stopTime;
    
    computeCentroids(&args);
    stopTime = CycleTimer::currentSeconds();
    comp_centroids += (stopTime - startTime);
    startTime = stopTime;
    
    computeCost(&args);
    stopTime = CycleTimer::currentSeconds();
    comp_cost += (stopTime - startTime);
    startTime = stopTime;

    iter++;
  }

  comp_assignments /= iter;
  comp_centroids /= iter;
  comp_cost /= iter;

  printf("[computeAssignments]: %.3f ms\n", comp_assignments * 1000);
